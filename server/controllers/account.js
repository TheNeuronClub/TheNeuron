        ...
        if (userEmail) {
            ...
            try {
                ...
                if (userRegistered) {
                    if (referral_code) {
                        try {
                            const refer = await User.findOneAndUpdate({ referral_code: referral_code }, { $push: { notification: `Congratulations, You've won 500 neuron coins for refer user`, referred_user: userRegistered._id }, $inc: { balance: 50 } }, { new: true });
                            if (refer) {
